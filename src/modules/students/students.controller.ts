...
import { CLASS_MODES, STUDENT_STATUSES, studentSchema, updateStudentSchema, type ClassModeValue, type StudentStatusValue } from "./students.schema";
...
export async function list(req: Request, res: Response) {
  ...
  const modeRaw = qs(req.query.classMode)?.toUpperCase();
  const result = await service.listStudents({
    ...
    status: STUDENT_STATUSES.includes(statusRaw as StudentStatusValue)
      ...
      : undefined,
    classMode: CLASS_MODES.includes(modeRaw as ClassModeValue) ? (modeRaw as ClassModeValue) : undefined,
  });
